...
def main(row_number):  
    ...
    # Create a PPO model
       RL = PPO("MlpPolicy", env, verbose = 1, n_steps = 2000)    # 500不够还在下降中 -1.10，max num of step() in an episode, regardless of terminate state of a episode
    # Train the model
    # total_timesteps is total number of step(), where n_steps of step() as a episode, after every n_steps calls reset() regardless of terminate state
       RL.learn(total_timesteps = 16000, progress_bar=True)     # 5000
